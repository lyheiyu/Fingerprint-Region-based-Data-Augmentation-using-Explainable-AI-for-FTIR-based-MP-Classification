...
if __name__ == '__main__':
    ...
    # model_file = 'Models/Conv1D_FTIR_modelThirdDataset.h5'
    ...
    i=0
    clusterdata1=[]
    heatforloop=heatmap4[:][:]
    j=0
    k=0
    # print('heatmap',heatforloop[0][1])
    # for i in range(len(heatmap4)):
    #     for j in range(len(heatmap4[i])):
    #         for m in range(len(heatmap4[i][j])):
    #             clusterdata1.append([xspace[m],heatmap4[i][j][m]])
    pid=4
    heatforloop2=heatmap4[pid]
    for j in range(len(heatmap4[pid])):
        for m in range(len(heatmap4[pid][j])):
            clusterdata1.append([xspace[m],heatmap4[pid][j][m]])

    j=0
    clusterresults=[]
    for item in heatforloop2:
        clusterresult=[]
        for j in range(len(item)):
            clusterresult.append([xspace[m],item[j]])
        clusterresults.append(clusterresult)
    clusterresults=np.array(clusterresults)
    print('clusterresults',clusterresults.shape)
    ...
    for i in range(len(heatmap4)):
        for j in range(len(heatmap4[i])):
            for k in range(len(heatmap4[i][j])):
                labelstotal.append(labels1[k])


    gmm=GMM(n_components=3,random_state=0).fit(clusterdata1)
    labels=gmm.predict(clusterdata1)

    # labels2=np.array(labels)
    # labels2=labels2.reshape(len(heatmap4[0]),len(heatmap4[0][1]))
    # for item in labels2:
    #     print(item)
    # print('labels2',labels2.shape)
    labeleaches=[]
    for item in clusterresults:
        gmmeach=GMM(n_components=3).fit(item)
        labeleach=gmmeach.predict(item)
        print(labeleach)
        labeleaches.append(labeleach)
    labeleaches=np.array(labeleaches)
    print(labeleaches.shape)
    labeleaches=labeleaches.reshape(clusterresults.shape[0]*clusterresults.shape[1], 1)
    print('labeleach',labeleaches.shape)
    # for item in labels:
    #     print(item)
    # print(labels)
    ...
    plt.show()
    plt.tick_params(labelsize=20)
    plt.scatter(clusterdata1[:, 0], clusterdata1[:, 1], c=labeleaches)
    plt.title('GaussianMixture cluster', font2)
    plt.xlim(max(waveLength) + 100, 400)
    plt.legend()
    plt.show()
    ...
    plt.xlim(max(waveLength) + 100, 400)
    plt.show()
    # plt.scatter(clusterdata1[:, 0], clusterdata1[:, 1], c=labelstotal)
    # plt.tick_params(labelsize=20)
