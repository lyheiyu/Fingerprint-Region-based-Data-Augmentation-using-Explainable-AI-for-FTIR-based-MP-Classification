...
def generatedataBySperateLSforEach9(wavelength,intensity,n):
    ...
    intensityforLoop=np.array(intensityforLoop)
        # print(intensityforLoop.shape)
    return intensityforLoop,y_add
def generatedataBySperateLSforEach10(wavelength,intensity,n,firstIdx,lastIdx,counts):
    order=4
    y_add=[]
    wavelength=np.array(wavelength)
    # index01 = np.where(wavelength < 800)
    ...
    #for mt in range(500):
    ####for the 4th dataset
    ####for the 1st dataset :300
    # random.seed(20)
    for mt in range(counts):
        s1 = random.choice(seprate1)
        s2 = random.choice(seprate2)
        s3 = random.choice(seprate3)
    ...
    intensityforLoop=np.array(intensityforLoop)
        # print(intensityforLoop.shape)
    return intensityforLoop,y_add
from FTIR_GAN3 import GAN
import tensorflow as tf
def normalize(arr: np.ndarray) -> np.ndarray:
    arr -= arr.min()
    arr /= arr.max()
    return arr
def generatedataBySperateLSforEach11(wavelength,intensity,n,firstIdx,lastIdx,counts):
    order=4
    y_add=[]
    wavelength=np.array(wavelength)
    # index01 = np.where(wavelength < 800)
    ...
    # random.seed(20)
    epochs = 50
    gan1 = GAN(len(seprate1[0]), seprate1)

    seprate1 = gan1.train(epochs=epochs, batch_size=20, sample_interval=1000)
    gan2 = GAN(len(seprate2[0]), seprate2)

    seprate2 = gan2.train(epochs=epochs, batch_size=20, sample_interval=1000)
    gan3 = GAN(len(seprate3[0]), seprate3)

    seprate3 = gan3.train(epochs=epochs, batch_size=20, sample_interval=1000)
    numSynth=20
    # random_latent_vectors1 = tf.random.normal(shape=(numSynth,len(seprate1[0])))
    # seprate1 = gan1(random_latent_vectors1)
    # print(seprate1.shape)
    # seprate1 = seprate1.numpy()
    # seprate1 = seprate1.reshape((numSynth, len(seprate1[0])))
    # for i in range(len(seprate1)):
    #     seprate1[i, :] = normalize(seprate1[i, :])
    # random_latent_vectors2 = tf.random.normal(shape=(numSynth, len(seprate2[0])))
    # seprate2= gan2(random_latent_vectors2)
    # print(seprate1.shape)
    # seprate2 = seprate2.numpy()
    # seprate2 = seprate2.reshape((numSynth, len(seprate2[0])))
    # for i in range(len(seprate1)):
    #     seprate2[i, :] = normalize(seprate2[i, :])
    # random_latent_vectors3 = tf.random.normal(shape=(numSynth, len(seprate3[0])))
    # seprate3 = gan3(random_latent_vectors3)
    # print(seprate3.shape)
    # seprate3 = seprate3.numpy()
    # seprate3 = seprate3.reshape((numSynth, len(seprate3[0])))
    for i in range(len(seprate1)):
        seprate1[i, :] = normalize(seprate1[i, :])
        seprate2[i, :] = normalize(seprate2[i, :])
        seprate3[i, :] = normalize(seprate3[i, :])
    for mt in range(counts):
        s1 = random.choice(seprate1)
        s2 = random.choice(seprate2)
        s3 = random.choice(seprate3)
